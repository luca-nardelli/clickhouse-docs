...
The next step is to get your current data into ClickHouse. We have lots of [table functions](./sql-reference/table-functions/)
and [integrations](./integrations) for ingesting data. We have some examples in the tabs below, or check out our [Integrations](./integrations)
...
Use the [`postgresql` table function](/en/sql-reference/table-functions/postgresql) to read data from a table in PostgreSQL. You can also connect to your ClickHouse server using a command-line tool named **clickhouse client**:
  ```bash
  ./clickhouse client
  ```
  If you get the smiley face prompt, you are ready to run queries!
